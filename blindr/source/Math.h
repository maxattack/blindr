...
struct vec2;
inline vec2 vec(float ax, float ay);
...
struct vec2 {
	...
	vec2 conjugate() const     { return vec(x,-y); }
	float radians() const      { return atan2f(y,x); }
	vec2 reflection() const    { return vec(y,x); }
	vec2 anticlockwise() const { return vec(-y, x); }
	vec2 clockwise() const     { return vec(y, -x); }
	vec2 normalized() const    { return (*this) / magnitude(); }

	vec2 operator+(vec2 q) const  { return vec(x+q.x, y+q.y); }
	vec2 operator-(vec2 q) const  { return vec(x-q.x, y-q.y); }
	vec2 operator*(vec2 q) const  { return vec(x*q.x, y*q.y); }
	vec2 operator/(vec2 q) const  { return vec(x/q.x, y/q.y); }

	vec2 operator-() const        { return vec(-x, -y); }
	vec2 operator*(float k) const { return vec(k*x, k*y); }
	vec2 operator/(float k) const { return vec(x/k, y/k); }
...
vec2 vec(float ax, float ay) {
	vec2 result;
	result.x = ax;
	result.y = ay;
	return result;
}
inline vec2 cmul(vec2 u, vec2 v) { return vec(u.x*v.x-u.y*v.y, u.x*v.y+u.y*v.x); }
inline vec2 cdiv(vec2 u, vec2 v) {
	float normInv = 1.0f/v.norm();
	return vec((u.x*v.x+u.y*v.y)*normInv, (v.x*u.y-u.x*v.y)*normInv);
}
inline vec2 polar(float radius, float radians) { return vec(radius*cosf(radians), radius*sinf(radians)); }
...
inline vec2 operator*(float k, vec2 q) { return vec(k*q.x, k*q.y); }
...
struct AABB;
inline AABB aabb(float x, float y, float w, float h);
...
struct ivec2;
inline ivec2 ivec(int x, int y);

struct ivec2 {
	int x, y;
	
	inline vec2 toFloat() { return vec(x, y); }
	inline ivec2 operator+(const ivec2& rhs) { return ivec(x+rhs.x, y+rhs.y); }
	inline ivec2 operator-(const ivec2& rhs) { return ivec(x-rhs.x, y-rhs.y); }
	inline ivec2 operator*(const ivec2& rhs) { return ivec(x*rhs.x, y*rhs.y); }
	inline ivec2 operator/(const ivec2& rhs) { return ivec(x/rhs.x, y/rhs.y); }
};
